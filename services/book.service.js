...
function query(filterBy = {}) {
  return storageService.query(BOOK_KEY).then((books) => {
    ...
    // filter by minprice so all the books with price lower than the minprice will be filtered out
...
function _createBook(title, price, description) {
  return {
...
function getDefaultFilter() {
  return { title: "", minPrice: 0, onSale: false }
}


function getRandomImg() {
  return `http://coding-academy.org/books-photos/${utilService.getRandomInt(
    1,
    20
  )}.jpg`
}